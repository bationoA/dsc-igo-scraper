"""This is the scraper class for the `UNICEF Middle East and North Africa`'s website. It collects all publications
...
class MiddleEastAndNorthAfricaScraper:
    _organization_acronym: str = "UNICEF"
    _organization_region: str = "Middle East and North Africa"
    ...
    def get_publications_details(self):
        ...
        while not last_page:
            ...
            page_url = self.get_page_url(page_number=page)
    ...
    def get_page_url(self, page_number: int) -> str:
        ...
        return f"https://www.unicef.org/mena/research-and-reports?page=%2C%2C{page_number}#listAnchor"
    ...
    def get_url(self, p_div: BeautifulSoup) -> str:
        try:
            p_ulr = p_div.find('div', class_="list-content").find('h5').text.strip()
            return add_base_url_if_missing(base_url=self.download_base_url, url=p_ulr)
        except:
            return ""
    ...
    def get_lang_link(self, p_div: BeautifulSoup) -> list:
        ...
        try:
            ...
            if not len(files_divs):
                p_url = self.get_doc_link(soup_page=p_div)
