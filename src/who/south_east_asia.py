"""This is the scraper class for `WHO South-East-Asia` website. It collects all publications and their details using
...
class WhoSouthEastAsiaScraper:
    _organization_acronym: str = "WHO"
    _organization_region: str = "South-East-Asia"
    ...
    def get_publications_list_from_api(self, skip: int = 0) -> dict:
        ...
        # API URL
        sf_site = "fbe30696-a1c9-4be0-87d1-fc262d0b0dee"  # Specific to South-East Asia
        sf_provider = "OpenAccessProvider"  # Specific to South-East Asia
        filter_ = "filter=publishingoffices%2Fany(x%3Ax%20eq%20678adf7e-24d8-4ef4-9da9-8c6ada4cf56b)"
        api_url = f"https://www.who.int/api/hubs/publications?sf_site={sf_site}" \
                  ...
                  f"TrimmedTitle&$format=json&$skip={skip}&$top={self.api_rqst_max_items}&${filter_}&$count=true"
        ""
