"""This is the scraper class for the ` WIPO's Global` website. It collects all publications
...
class WipoGlobalScraper:
    _organization_acronym: str = "WIPO"
    _organization_region: str = "Global"
    _download_base_url = "https://www.wipo.int"
    ...
    def get_all_publications_links(self):
        ...
        while not last_page:
            ...
            publication_urls_divs = current_page_soup.find_all("div", class_="media line")
            if not publication_urls_divs:
                break
            ...
            if page == self.starting_page:
                ...
                self.max_pb_per_page = len(publication_urls_divs)
            ...
            try:
                publ_links = [add_base_url_if_missing(base_url=self.download_base_url, url=h5.find("a").get("href"))
                              for h5 in publication_urls_divs]
            except:
                publ_links = []
    ...
    def get_publication_date(self, soup_page: BeautifulSoup) -> str:
        try:
            ps = soup_page.find_all("p")
            date_year = [p for p in ps if "Publication year: " in p.text]  # Publication year: 2023
            date_year = date_year[0].text
            y = date_year.split(":")[1].strip()
            y = int(y)
            d, m = 1, 1  # Default values
    ...
    def get_links_n_lang_from_page(self, soup_page: BeautifulSoup):
        ...
        list_links = [
            {
                'lang': format_language(lang=clean_text(atag.text)),
    ...
    def get_page_url(self, page_number: int) -> str:
        ...
        """
        url = "https://www.wipo.int/publications/en/search.jsp?start={}&rows={}&lang=EN&sort=pubDate"

        return url.format(self.max_pb_per_page * page_number, self.max_pb_per_page)
